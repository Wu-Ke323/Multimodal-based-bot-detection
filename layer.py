...
class RGATLayer(torch.nn.Module):
    def __init__(self, num_edge_type, in_channel, out_channel, GAT_heads, semantic_head, dropout):
        ...
        for i in range(int(num_edge_type)):
            self.GAT_list.append(GATConv(in_channels=in_channel, out_channels=out_channel, heads=GAT_heads, dropout=dropout,concat=False))
    ...
    def forward(self, features, edge_index, edge_type):
        ...
        u = self.GAT_list[0](features, edge_index_list[0].squeeze(0)).flatten(1) #.unsqueeze(1)
        ...
        for i in range(1,len(edge_index_list)):
            ...
            u = self.GAT_list[i](features, edge_index_list[i].squeeze(0)).flatten(1)
            a = self.gate(torch.cat((u, features), dim = 1))
